...
add_action( 'test/before_setup', function ( $file, $class ) {
	add_action( 'plugins_loaded', function () use ( $file, $class ) {
		...
		\Test\test()->get( $file, $class )->loaders()->add( 'debug_bars', [
			'registry' => '\Test_Debug_Bar_Extension\Loaders\Debug_Bars',
		] );
	} );
}, 3, 2 );

// When a logger instance is added, also add a debug bar instance.
add_action( 'test/loader_registered', function ( $key, $value, $loader, $parent_id ) {
	add_action( 'plugins_loaded', function () use ( $parent_id, $loader, $key ) {
		...
		if ( $loader === 'Test_Logger\Loaders\Logger' ) {
			$name = Test::get_by_id( $parent_id )->name;
			...
			$class = 'Test_Debug_Panel_Instance_' . md5( $key . '_' . $parent_id );
			eval( 'class ' . $class . ' extends Test_Debug_Bar_Panel_Logger_Instance{}' );

			Test::get_by_id( $parent_id )->debug_bars()->add( $key, [
				'class' => $class,
				'args'  => [
					'title'      => "$name ${key}s",
					'parent_id'  => $parent_id,
...
add_action( 'test/logger/after_logged_item', function ( \Test_Logger\Factories\Log_Item $logged_item, $logger ) {
