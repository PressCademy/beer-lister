...
namespace Test_Logger\Abstracts;


use ArrayIterator;
use Exception;
use Test_Logger\Factories\Log_Item;
use WP_Error;
...
abstract class Event_Type extends ArrayIterator {
	 ...
	 */
	public $writer_class = 'Test_Logger\Factories\Basic_Logger';
	 ...
	 */
	public $log_item_class = 'Test_Logger\Factories\Log_Item';
	...
	public function __construct() {
		 ...
		 */
		$this->writer_class = apply_filters( 'test/event_type/writer_class', $this->writer_class, $this->type, $this->log_item_class );
		 ...
		 * @param string $writer_class   The writer class to instantiate when a writer is created.
		 */
		$this->log_item_class = apply_filters( 'test/event_type/log_item_class', $this->log_item_class, $this->type, $this->writer_class );
		 ...
		 */
		$this->capabilities   = apply_filters( 'test/event_type/capabilities', $this->capabilities, $this->type );
		 ...
		 * @param type  $group        The current event group
		 */
		$this->group = apply_filters( 'test/event_type/group', $this->group, $this->type );
		 ...
		 * @param type  $group        The current event group
		 */
		$this->volume = apply_filters( 'test/event_type/volume', $this->volume, $this->type );
	}
	...
	public function writer() {
		...
		} elseif ( true === test()->logger()->is_muted() ) {
			return new WP_Error(
				'events_are_muted',
		...
		if ( ! is_subclass_of( $this->writer_class, 'Test_Logger\Abstracts\Writer' ) ) {
			return new WP_Error(
	...
	public function log( $code, $message, $data = array() ) {
		...
		do_action( 'test/logger/after_logged_item', $item, $this );
