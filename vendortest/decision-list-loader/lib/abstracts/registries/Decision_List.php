 ...
 *
 * @since   1.0.0
 * @package Test\Abstracts\Registries
 */


namespace Test_Decision_Lists\Abstracts\Registries;
...
abstract class Decision_List extends Loader_Registry {
	 ...
	 * @inheritDoc
	 */
	protected $abstraction_class = 'Test_Decision_Lists\Abstracts\Decision';

	protected $default_factory = 'Test_Decision_Lists\Factories\Decision_Instance';
		...
		if ( false === $result ) {
			...
			foreach ( $this as $decision ) {
				...
				if ( is_wp_error( $valid ) ) {
					...
					test()->logger()->log_wp_error( 'notice', $valid );
			...
			if ( ! isset( $decision ) ) {
				$decision = test()->logger()->log_as_error(
					'error',
