...
use Test\Traits\Test_Templates;
use WP_Error;
...
abstract class Settings_Field {
	...
	public function __construct( $value, array $params ) {
		...
		$errors = test()->logger()->gather_errors(
	...
	public function get_field_param( $param ) {
		...
		return new WP_Error(
			'invalid_field_type',
			__( 'The requested param is not a valid param for this field.', 'test' ),
			[
				'param'            => $param,
	...
	protected function get_admin_template_path( $template_name ) {
		return trailingslashit( test()->dir() ) . 'templates/admin/settings-fields/' . $template_name . '.php';
	}
