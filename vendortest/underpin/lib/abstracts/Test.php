...
namespace Test\Abstracts;

use Exception;
use Test\Factories\Loader_Registry;
use WP_Error;
...
abstract class Test {
	 ...
	 *
	 * @since 1.0.0
	 * @var Test|null The one true instance of the Test
	 */
	 ...
	 * fatal errors.
	 *
	 * @return \Test_Logger\Loaders\Logger|WP_Error
	 */
	...
	public static function is_debug_mode_enabled() {
		...
		$debug_enabled_with_querystring = isset( $_GET['test_debug'] ) && '1' === $_GET['test_debug'];
		...
		return apply_filters( 'test/debug_mode_enabled', false, get_called_class() );
	}
	...
	public static function export() {
		...
		foreach ( Test::$instances as $key => $instance ) {
			if ( $instance instanceof Test ) {
				$results = Test::get_by_id( $key )->export_registered_items( $results );
			}
		}
	...
	public function below_version_notice() {
		...
		if ( version_compare( $wp_version, $this->minimum_wp_version, '<' ) ) {
			echo '<div class="error">
							<p>' . __( sprintf( "Test plugin is not activated. The plugin requires at least WordPress %s to function.", $this->minimum_wp_version() ), 'test' ) . '</p>
						</div>';
		}

		if ( version_compare( phpversion(), $this->minimum_php_version, '<' ) ) {
			echo '<div class="error">
							<p>' . __( sprintf( "Test plugin is not activated. The plugin requires at least PHP %s to function.", $this->minimum_php_version() ), 'test' ) . '</p>
						</div>';
		}
	}
	...
	protected function unsupported_actions() {
		...
		self::$instances[ __CLASS__ ] = new WP_Error(
			...
			__( sprintf(
				"The Test plugin requires at least WordPress %s, and PHP %s.",
	...
	protected function setup() {
		 ...
		 * @since 1.0.0
		 */
		do_action( 'test/before_setup', $this->file(), get_called_class() );
		 ...
		 * @since 1.0.0
		 */
		do_action( 'test/after_setup', $this->file(), get_called_class() );
	}
	 ...
	 */
	public static function make_class( $value = [], $default_factory = 'Test\Factories\Test_Instance' ) {
	...
	/**
	 * Fetch an Test Instance by the registry key.
	 *
	 * @since 1.2
	 *
	 * @param string $key The instance key.
	 *
	 * @return Test|WP_Error The test instance if found, otherwise WP_Error.
	 */
	public static function get_by_id( $key ) {
		...
		return new WP_Error(
			'instance_not_found',
			'The instance key provided is not associated with an Test instance',
			[ 'key' => $key ]
		);
	}
