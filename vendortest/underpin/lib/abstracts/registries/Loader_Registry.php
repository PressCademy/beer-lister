...
namespace Test\Abstracts\Registries;

use Test\Abstracts\Feature_Extension;
use Test\Abstracts\Test;
use Test\Traits\With_Parent;
...
abstract class Loader_Registry extends Registry {
	...
	public function add( $key, $value ) {
		...
		if ( true === $valid ) {
			$this[ $key ] = Test::make_class( $value, $this->default_factory );
		} else {
			$this[ $key ] = $valid;
		}

		// If this implements registry actions, go ahead and start those up, too.
		if ( self::has_trait( 'Test\Traits\Feature_Extension', $this->get( $key ) ) ) {
			$this->get( $key )->do_actions();

			if ( ! $this instanceof \Test_Logger\Loaders\Logger && ! is_wp_error( test()->logger() ) ) {
				test()->logger()->log(
					'debug',
					'loader_actions_ran',
		...
		if ( ! is_wp_error( $valid ) ) {
			do_action( 'test/loader_registered', $key, $value, get_called_class(), $this->parent_id );
		}
