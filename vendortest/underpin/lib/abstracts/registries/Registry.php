...
abstract class Registry extends ArrayIterator {
	...
	public function add( $key, $value ) {
		...
		if ( true === $valid ) {
			 ...
			 */
			do_action( 'test/registry/after_added_item', $this->registry_id, $key, $value );
			if ( ! is_wp_error( test()->logger() ) ) {
				test()->logger()->log(
					'debug',
					'valid_event_added',
	...
	public function get( $key ) {
		if ( isset( $this[ $key ] ) ) {
			...
			if ( 'logger' !== $key && ! is_wp_error( test()->logger() ) && test()->is_debug_mode_enabled() ) {
				test()->logger()->log_wp_error( 'warning', $error );
			}
