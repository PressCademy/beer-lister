...
use Test\Abstracts\Registries\Registry;
...
class Loader_Registry extends Registry {
	...
	public function get( $key ) {
		...
		if ( is_array( $valid ) && isset( $valid['registry'] ) && is_string( $valid['registry'] ) ) {
			/* @var $valid \Test\Abstracts\Registries\Loader_Registry */
	...
	protected function validate_item( $key, $value ) {
		...
		if ( ! is_wp_error( test()->logger() ) && $errors->has_errors() ) {
			test()->logger()->log_wp_error( 'error', $errors );
		}
