...
trait Templates {
	...
	public function get_template( $template_name, array $params = [] ) {
		...
		if ( $this->is_valid_template( $template_name ) ) {
			...
			if ( $this->template_file_exists( $template_name ) ) {
				...
				$template      = test()->logger()->log_as_error(
					'error',
					'template_file_does_not_exist',
					__( "Template $template_name was not loaded because the file located at $template_path does not exist.", 'test' )
				);

				/**
				 * Fires just after the template loader determines that the template file does not exist.
				 */
				do_action( 'test/templates/invalid_template_file_doesnt_exist', $template_name, $params, $template_path, $template );
			}
		} else {
			$template = test()->logger()->log_as_error(
				'error',
			 ...
			 */
			do_action( 'test/templates/invalid_template_not_in_schema', $template_name, $params, $template );
		}
	...
	private function get_template_arg( $template_name, $arg ) {
		if ( isset( $this->get_template_arg_defaults()[ $arg ] ) ) {
			if ( $this->is_valid_template( $template_name ) ) {
				...
				$result = test()->logger()->log_as_error(
					'error',
					'test_get_arg_invalid_template',
					__( "Template $template_name argument $arg was not fetched because $template_name is not a valid template.", 'test' )
				);
			}
		} else {
			$result = test()->logger()->log_as_error(
				'error',
				'test_get_arg_invalid_argument',
				__( "Template $template_name argument $arg was not fetched because $arg is not a valid template argument.", 'test' )
			);
		}
	...
	private function include_template( $template_name, $params ) {
		...
		if ( 'private' !== $this->get_template_arg( $template_name, 'override_visibility' ) ) {
			$this->params[ $this->depth ] = apply_filters( "test/templates/template_params", $params, $template_name, $this->get_template_path( $template_name ), $this->depth );
		} else {
		...
		if ( 'private' !== $this->get_template_visibility( $template_name ) ) {
			 ...
			 */
			do_action( 'test/templates/before_template_buffer', $template_name, $this->depth, $this->params[ $this->depth ], $this->get_template_path( $template_name ) );
		}
		...
		if ( 'private' !== $this->get_template_visibility( $template_name ) ) {
			 ...
			 */
			do_action( 'test/templates/before_template', $template_name, $this->depth, $this->params[ $this->depth ], $this->get_template_path( $template_name ) );
		}

		test_include_file_with_scope( $this->locate_template( $template_name ), [
			'template' => $this,
		] );
		...
		if ( 'private' !== $this->get_template_visibility( $template_name ) ) {
			 ...
			 * Note that this only fires when the provided template is not private.
			 */
			do_action( 'test/templates/after_template', $template_name, $this->depth, $this->params[ $this->depth ], $this->get_template_path( $template_name ) );
		}
		...
		if ( 'private' !== $this->get_template_visibility( $template_name ) ) {
			 ...
			 * Note that this only fires when the provided template is not private.
			 */
			do_action( 'test/templates/after_template_buffer', $template_name, $this->depth, $this->params[ $this->depth ], $this->get_template_path( $template_name ) );
		}
 ...
 */
function test_include_file_with_scope( $file, $scope ) {
	if ( file_exists( $file ) ) {
		extract( $scope );
		include $file;
