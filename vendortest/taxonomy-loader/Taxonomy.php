...
namespace Test_Taxonomies\Abstracts;


use Test\Traits\Feature_Extension;
...
abstract class Taxonomy {
	...
	public function register() {
		...
		if ( is_wp_error( $registered ) ) {
			test()->logger()->log_wp_error( 'error', $registered );
		} else {
			test()->logger()->log(
				'notice',
				'registered_taxonomy',
	...
	public function save( array $args ) {
		...
		if ( is_wp_error( $saved ) ) {
			test()->logger()->log_wp_error( 'error', $saved );
		} else {
			test()->logger()->log(
				'notice',
				$this->id . '_saved',
	...
	public function delete( $term, $args = [] ) {
		...
		if ( false === $deleted ) {
			...
			test()->logger()->log_wp_error( 'warning', $deleted );
		} elseif ( is_wp_error( $deleted ) ) {
			test()->logger()->log_wp_error( 'error', $deleted );
		}
